/*
 *  stereoBlobTrackEtc.cpp
 *
 *MODIFIED BY MAJURE
 *no extra inputs
 *extra params:
 *	track -- whether to actually track or to just report on whether blob found, and where if so (D true)
 *extra outputs:
 *	/stereoBlobTrack/fieldLoc:o -- divergence calculated for gaze stopping, for use as a measure of vismotor map error. NULL if nothing found
 *
 * 	Logan Niehaus
 * 	6/28/11
...
public:
	...
	strBallLocThread(ResourceFinder &_rf) :
		RateThread(50), rf(_rf) {
	}
	...
	virtual bool threadInit() {
		...
		if (((int) rf.check("red") + (int) rf.check("green") + (int) rf.check(
				"blue") + (int) rf.check("yellow")) > 1) {
		...
		string trkstr = rf.check("track", Value("yes")).asString().c_str();
		if (trkstr == "yes") {
			track = true;
		} else {
			track = false;
		...
		if (robot == "icubSim") {
			...
			printf(
					"No robot name specified, using real iCub default trajectory times\n");
			neckTT = rf.check("nt", Value(1.0)).asDouble();
			eyeTT = rf.check("et", Value(0.5)).asDouble();
		}
		...
		portErr = new BufferedPort<Bottle>;
		string portErrName = "/" + name + "/fieldLoc:o";
		portErr->open(portErrName.c_str());

		readySig = new Port;
		readySig->open("/stereoBlobTrack/fine:i");

		//start up gaze control client interface
		Property option("(device gazecontrollerclient)");
		option.put("remote", "/iKinGazeCtrl");
		option.put("local", "/clientT/gaze");
	...
	virtual void run() {
		...
		if (pImgL && pImgR) {
			...
			for (int lr = 0; lr < 2; lr++) {
				...
				T = new Mat(oImg->height(), oImg->width(), CV_32F,
						(void *) oImg->getRawImage());
				...
				findContours(*X, contours, hierarchy, CV_RETR_LIST,
				...
				for (int i = 0; i < contours.size(); i++) {
					...
					if (abs(contourArea(Mat(contours[i]))) / arcLength(Mat(
							contours[i]), true) > maxSize && abs(contourArea(
							Mat(contours[i]))) / arcLength(Mat(contours[i]),
							true) > 2.0) {
			...
			if (loc.size() == 4) {
				...
				if (sqrt(du * du + dv * dv) < tol) {
					if(track) {
						...
						//put something here
						Bottle &div = portErr->prepare();
						div.addDouble(sqrt(du*du+dv*dv));
						portErr->write();
						portErr->unprepare();
					}
				} else {
