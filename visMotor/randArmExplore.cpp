...
int main(int argc, char *argv[]){
	...
	BufferedPort<Vector> armLocCart;
	armPlan.open("/randArm/plan");
	armPred.open("/randArm/pred");
	armLocJ.open("/randArm/act");
	armLocCart.open("/randArm/cart");
	bool fwCvOn = 0;
    ...
    while (true){
    	...
    	if (rad > 0.3){
    		...
    		armJ = encoders;
    		armLocJ.write();
    		printf("Cartesian: %.1lf %.1lf %.1lf\n", commandCart[0], commandCart[1], commandCart[2]);
    		Vector &armC = armLocCart.prepare();
    		armC = commandCart;
    		armLocCart.write();
    	}
    	else{
    ...
    robotDevice.close();
    gsl_rng_free(r);
    armPlan.close(); armPred.close(); armLocJ.close(); armLocCart.close();
    return 0;
}
