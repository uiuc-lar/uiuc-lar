...
public:
    ...
    virtual bool threadInit()
    {
        ...
        printf("%f %f %f %f\n",N(0,0),N(0,1),N(N.rows()-1,0),N(N.rows()-1,1));
    ...
    virtual void run()
    {
        ...
        if (pImgIn)
        {
        	...
        	imgOut.copy(*pImgIn);
        	sMap->resize(pImgIn->width(),pImgIn->height());
        	sMap->zero();
        	...
        	if (featVec) {
        		...
        		//don't try to fit a crummy ellipse
        		if (nimp.size() >= 6) {
